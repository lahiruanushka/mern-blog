...
const LoginPrompt = ({ isOpen = false, onClose = () => {} }) => {
  // Animation variants
  ...
  const handleSignIn = () => {
    alert("Sign In clicked - implement navigation to /signin");
    onClose();
  };

  const handleRegister = () => {
    alert("Register clicked - implement navigation to /signup");
    onClose();
  };
  ...
  return (
    <AnimatePresence mode="wait">
      {isOpen && (
        <>
          {/* Fixed Modal Portal with proper z-index */}
          <motion.div
            initial="hidden"
            animate="visible"
            exit="exit"
          ...
          >
            {/* Backdrop with blur - Fixed click handler */}
            <div
            ...
            <motion.div
              ...
              className="relative max-w-md w-full mx-auto z-10"
              onClick={(e) => e.stopPropagation()} // Prevent backdrop close when clicking modal
            >
              {/* Modal Card */}
              <motion.div
                variants={modalVariants}
                className="relative bg-white/95 dark:bg-gray-900/95 backdrop-blur-md rounded-3xl shadow-2xl border border-white/20 dark:border-gray-700/20 overflow-hidden w-full max-w-md"
              >
                ...
                <div className="absolute bottom-0 left-0 w-24 h-24 bg-gradient-to-tr from-pink-500/10 to-orange-500/10 rounded-full blur-2xl pointer-events-none" />
                <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-40 h-40 bg-gradient-to-r from-indigo-500/5 via-purple-500/5 to-pink-500/5 rounded-full blur-3xl pointer-events-none" />
                ...
                <motion.button
                  ...
                  onClick={handleClose}
                  type="button"
                  className="absolute top-4 right-4 z-50 p-2 bg-white/20 dark:bg-gray-800/50 backdrop-blur-sm rounded-xl text-gray-600 dark:text-gray-400 hover:text-gray-800 dark:hover:text-gray-200 hover:bg-white/30 dark:hover:bg-gray-800/70 transition-all duration-200 shadow-lg border border-white/10 dark:border-gray-700/30 cursor-pointer"
                >
                  <X className="w-5 h-5" />
                </motion.button>
                ...
                <div className="relative z-10 p-8">
                  ...
                  >
                    <motion.div
                      ...
                      className="relative"
                    >
                      <div className="absolute inset-0 bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 rounded-2xl blur-xl opacity-50 pointer-events-none"></div>
                      <div className="relative p-6 bg-gradient-to-br from-indigo-600 via-purple-600 to-pink-600 rounded-2xl shadow-2xl">
                        <Lock className="w-10 h-10 text-white" />
                      </div>
                      {/* Floating sparkles */}
                      <div className="absolute -top-2 -right-2 w-6 h-6 bg-gradient-to-r from-yellow-400 to-orange-500 rounded-full flex items-center justify-center shadow-lg pointer-events-none">
                  ...
                  >
                    Unlock the full potential of our platform and join a
                    community of curious minds sharing knowledge, one byte at a
                    time.
                  </motion.p>
                  ...
                  >
                    {features.map((feature, index) => (
                      ...
                      >
                        <div
                          className={`flex items-center justify-center w-12 h-12 rounded-xl bg-gradient-to-r ${feature.gradient} shadow-lg group-hover:shadow-xl transition-shadow duration-300`}
                        >
