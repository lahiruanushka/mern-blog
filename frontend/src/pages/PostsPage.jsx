...
const PostsPage = () => {
  ...
  const categories = ['all', 'Technology', 'Lifestyle', 'Business', 'Science', 'Health', 'Travel', 'Food'];
  const sortOptions = [
  ...
  useEffect(() => {
    ...
    switch (sortBy) {
      ...
      case 'popular':
        ...
        break;
      case 'liked':
        filtered.sort((a, b) => (b.numberOfLikes || b.likes?.length || 0) - (a.numberOfLikes || a.likes?.length || 0));
        break;
      default: // latest
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex items-center justify-center min-h-[400px]">
              ...
              >
                ...
                <h2 className="text-2xl font-bold text-slate-900 dark:text-white mb-4">Loading Posts</h2>
                <p className="text-slate-600 dark:text-slate-400">Fetching the latest articles for you...</p>
              </motion.div>
            </div>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            ...
            <p className="text-xl text-slate-600 dark:text-slate-400 max-w-3xl mx-auto leading-relaxed">
              Explore our collection of insightful articles, stories, and ideas. One byte, one thought at a time.
            </p>
          </motion.div>
          ...
          >
            ...
            <div className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-lg rounded-2xl p-6 border border-white/50 dark:border-slate-700/50 text-center shadow-lg">
              ...
              <p className="text-2xl font-bold text-slate-900 dark:text-white">{new Set(posts.map(p => p.author?.name || p.userId)).size}</p>
            ...
            <div className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-lg rounded-2xl p-6 border border-white/50 dark:border-slate-700/50 text-center shadow-lg">
              ...
              <p className="text-2xl font-bold text-slate-900 dark:text-white">{filteredPosts.length}</p>
            ...
            >
              <p className="text-slate-600 dark:text-slate-400">
                {filteredPosts.length > 0 ? (
                  <>
                    Found <span className="font-semibold text-slate-900 dark:text-white">{filteredPosts.length}</span> articles
                    {searchTerm && <span> for "<span className="font-semibold text-blue-600 dark:text-blue-400">{searchTerm}</span>"</span>}
                    {selectedCategory !== 'all' && <span> in <span className="font-semibold text-purple-600 dark:text-purple-400">{selectedCategory}</span></span>}
                  </>
                ) : (
